...
namespace IonicCustomMod.Zip
{
    public partial class ZipFile
    {
        ...
        public static bool CheckZipPassword(string zipFileName, UInt32 key1, UInt32 key2, UInt32 key3)
        {
            // workitem 13664
            bool success = false;
            try
            {
                using (ZipFile zip1 = ZipFile.Read(zipFileName))
                {
                    foreach (var e in zip1)
                    {
                        ...
                        {
                            e.ExtractWithPassword(System.IO.Stream.Null, key1, key2, key3);
                        }
                    }
                }
                success = true;
            }
            catch(IonicCustomMod.Zip.BadPasswordException) { }
            return success;
        }
